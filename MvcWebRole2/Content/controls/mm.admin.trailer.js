var Trailer = function () {
    Trailer.prototype.GetTrailerGrid = function (trailerList) {
        ...
        var addButton = $("<div/>").attr("title", "add new trailer").attr("class", "btn btn-success").html("<b>+</b>").attr("style", "position: absolute;top: 12px;right: 2px;float: left;").attr("data-toggle", "modal").attr("data-target", "#myModal");
        ...
        for (i = 0; i < trailerList.length; i++) {
            ...
            if (trailerList[i].YoutubeURL != null && trailerList[i].YoutubeURL != undefined && trailerList[i].YoutubeURL != "")
                linkText = trailerList[i].YoutubeURL + " <a href=\"#\" data-toggle=\"modal\" data-target=\".bs-example-modal-lg\" onclick=\"new Trailer().PopulatePopup(" + i + ");\">change<a/>";
    ...
    Trailer.prototype.PopulatePopup = function (counter) {
        ...
        $("#myModalLabel").append($("<input/>").attr("type", "hidden").attr("id", "hf-trailer-id3").val("#trailer-row-data3_" + counter));
