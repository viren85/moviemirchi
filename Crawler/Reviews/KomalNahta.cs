...
namespace Crawler.Reviews
{
    public class KomalNahta
    {
        ...
        public ReviewEntity Crawl(string url, string affiliation)
        {
            ...
            {
                Debug.WriteLine(string.Format("Exception occored while getting reviews (Komal Nahta's Blog), message= {0}", ex.Message));
            }
        ...
        public ReviewEntity PopulateReviewDetail(string html, string affiliation)
        {
            ...
            if (htmlDoc.DocumentNode != null)
            {
                ...
                {
                    var headerNode = helper.GetElementWithAttribute(bodyNode, "p", "align", "center");
                    //HtmlNode head = headerNode.SelectSingleNode("h1");
                    var header = headerNode == null ? string.Empty : headerNode.InnerText;

                    var reviewerName = helper.GetElementWithAttribute(bodyNode, "div", "id", "site-description");
