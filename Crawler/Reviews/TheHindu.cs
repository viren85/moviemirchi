...
namespace Crawler.Reviews
{
    public class TheHindu
    {
        ...
        public ReviewEntity Crawl(string url, string affiliation)
        {
            ...
            {
                Debug.WriteLine(string.Format("Exception occored while getting reviews (The Hindu), message= {0}", ex.Message));
            }
        ...
        public ReviewEntity PopulateReviewDetail(string html, string affiliation)
        {
            ...
            if (htmlDoc.DocumentNode != null)
            {
                ...
                {
                    var headerNode = helper.GetElementWithAttribute(bodyNode, "h1", "class", "detail-title");
                    //HtmlNode head = headerNode.SelectSingleNode("h1");
                    var header = headerNode == null ? string.Empty : headerNode.InnerText;

                    
                    var reviewerName = helper.GetElementWithAttribute(bodyNode, "span", "class", "author");
                    var reviewName = reviewerName == null ? string.Empty : reviewerName.InnerText;
                    reviewName = reviewName.Trim();

                    var reviewContentNode = helper.GetElementWithAttribute(bodyNode, "div", "class", "article-text");
